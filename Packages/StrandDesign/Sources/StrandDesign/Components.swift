...
public enum NoopMetrics {
    // OVERRIDE POINT (bknoop fork): `var`, not `let` — same override-layer pattern as StrandPalette/
    // StrandFont, so a fork can retheme the card radius without touching this package's source.
    public static var cardRadius: CGFloat = 22   // Apple x WHOOP rounded cards — matches the liquid home card (LiquidTodayView.card)   // Apple x WHOOP: rounded cards
    // OVERRIDE POINTS (bknoop fork): the Hearth mockup's radius scale distinguishes a card (26) from a
    // divided list-row card (24), a chip (20) and a widget/glance tile (34) — one flat `cardRadius`
    // can't express all four. `var`s, same pattern as `cardRadius` above, so `HearthTheme` can set them;
    // default values equal the pre-fork radii (`cardRadius`/`pillRadius`/`cardRadius`) so a fork that
    // never touches these sees no change.
    public static var rowCardRadius: CGFloat = 22
    public static var chipRadius: CGFloat = 999
    public static var widgetRadius: CGFloat = 22
