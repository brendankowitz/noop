...
// MARK: - Today entry point (Hearth "Action" card)
//
// The mockup's card vocabulary maps the ManualWorkoutSheet's ENTRY POINT — not the sheet's contents —
// to the ink "Action" card: circular glyph, title, subtitle, chevron. Before this the sheet had no
// Today-visible trigger at all (only the "Add workout" pill on the Workouts screen), so the row the
// mockup draws in the Today stack is added here as its own leaf and rendered by `LiquidTodayView`'s
// Last-workouts section. It runs the SAME save flow the Workouts screen uses — `saveManualWorkout`,
// then `IntelligenceEngine.analyzeRecent()` so the new row is scored from the strap's own HR for that
// window straight away, then a repo refresh — so a workout logged from Today is indistinguishable
// from one logged from Workouts.

struct ManualWorkoutEntryCard: View {
    ...
    var body: some View {
        ActionCard(icon: "plus",
                   title: "Log a workout",
                   subtitle: "Add one you tracked elsewhere.") {
            showSheet = true
        }
        ...
        .sheet(isPresented: $showSheet) {
            ManualWorkoutSheet(editing: nil) { row, replacing in
                Task {
                    ...
                    // #598 parity with WorkoutsView: rescore the just-added workout from the strap's HR
                    // for its window NOW rather than waiting for the next analyze tick. No-ops when the
                    // strap had no HR for that window, and never overrides a value typed by hand.
                    await model.intelligence.analyzeRecent()
                    await repo.refresh()
                }
                // The sheet dismisses itself on save (`ManualWorkoutSheet.save()` calls `dismiss()`),
                // which clears `showSheet` — no explicit teardown needed here.
            }
        }
    }
}
